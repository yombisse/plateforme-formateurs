                ...
                <article class="overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5">
                    ...
                    <div class="p-4 sm:p-6">
                        ...
                        <div class="mt-4 sm:mt-6 flex flex-wrap gap-2">
                            ...
                            <a href="{{ route('admin.formation.poster', $formation->slug) }}" class="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-3 py-2 text-xs sm:text-sm font-medium text-slate-700 hover:bg-slate-50 transition">Générer affiche</a>
                            @if($formation->status === 'Brouillon')
                                <form action="{{ route('admin.formation.publish', $formation->slug) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="inline-flex items-center justify-center rounded-full bg-emerald-600 px-3 py-2 text-xs sm:text-sm font-medium text-white hover:bg-emerald-700 transition">Publier</button>
                                </form>
                            @else
                                <form action="{{ route('admin.formation.unpublish', $formation->slug) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-3 py-2 text-xs sm:text-sm font-medium text-slate-700 hover:bg-slate-50 transition">Dépublier</button>
                                </form>
                            @endif
                            <form action="{{ route('admin.formation.destroy', $formation->slug) }}" method="POST" class="inline" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cette formation ?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="inline-flex items-center justify-center rounded-full border border-red-200 bg-red-50 px-3 py-2 text-xs sm:text-sm font-medium text-red-600 hover:bg-red-100 transition">Supprimer</button>
                            </form>
                        </div>
                    </div>
                </article>
